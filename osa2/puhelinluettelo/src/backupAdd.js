...
const App = () => {
  ...
  const [addDataBool, setAddDataBool] = useState(false);
  ...
  const handleNameChange = (event) => {
    ...
    if (name.includes(event.target.value.toLowerCase())) {
      event.preventDefault();
      setAddDataBool(false);
      return alert(`${event.target.value} is already added to phonebook, replace the old number with the new one?`);
    } else {
      setAddDataBool(true);
    }
  };
